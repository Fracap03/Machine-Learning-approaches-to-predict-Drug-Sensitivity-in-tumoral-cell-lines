...
source('footprint_functions_Francesco.R')

#Reading file
transcriptomic <- read_csv('transcriptomic_zscore.csv')
...
transcriptomic <- transcriptomic[!is.na(transcriptomic$gene.name), ]
...
colnames(transcriptomic) <- sapply(colnames(transcriptomic), extract_before_underscore)
colnames(transcriptomic)[1] <- "genes.name"



gene_name <- transcriptomic[['genes.name']]
...
for (i in 2:ncol(transcriptomic)) {
  ...
  cell <- colnames(transcriptomic)[i]  
  print(cell)
  nes_dataset <- data.frame(
    gene_name = tf_activity_foot$gene_name 
  )
...
write.csv(merged_results, file = "merged_results_trans.csv", row.names = FALSE)
