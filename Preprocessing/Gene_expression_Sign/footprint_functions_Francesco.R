...
#'                      reg_minsize = 10,
#'                      integrated_regulons = FALSE)
#' output_uniprot <- convert_gene_name_in_uniprotid(viper_output$sign, organism = 'human')
#' output_uni_mf <- molecular_function_annotation(output_uniprot, organism = 'human')
#' filtered_tfs <- filter_VIPER_output(inferred_proteins_mf = output_uni_mf, "tfea")
#'
#' }
#' @export
...
#'   If `TRUE`, only significant analytes (where `significant == '+'`) are included.
#' @param significant Logical, whether to return only significant activity modulations. Default: `TRUE`.
#' @param integrated_regulons Logical, indicating whether to use regulons
#'   derived from experimental data (Ser/Thr and Tyr Kinome Atlas (PMIDs: 36631611, 38720073)). Default: `FALSE`.
#' @param collectri Logical, indicating whether to use CollecTRI regulons
#'      (only applicable to  `"tfea" `). Default: `FALSE`.
...
#' tfea_df <- inferred_proteins <- run_footprint_based_analysis(omic_data = tr_df,
#'                                                              analysis = "tfea",
#'                                                              organism = "human",
#'                                                              regminsize = 10,
...
#' ksea_df <- inferred_proteins <- run_footprint_based_analysis(omic_data = phos_df,
#'                                                              analysis = "ksea",
#'                                                              organism = "human",
#'                                                              regminsize = 5,
#'                                                              hypergeometric_corr = TRUE,
...
#' }
#' @export
run_footprint_based_analysis_Francesco <- function(omic_data,
                                                   analysis,
                                                   organism,
                                                   reg_minsize,
                                                   exp_sign,
  ...
  if (correct_proteomics) {
    ...
    viper_on_prot <- dplyr::left_join(viper_all,
                                      prot_df,
                                      by = c('gene_name')) %>%
  ...
  if (hypergeom_corr) {
    ...
    ea_output <- weight_viper_score(run_hypergeometric_test(omic_data = omic_data,
                                                         ...
                                                         custom = custom,
                                                         custom_path = custom_path))
    output_uniprot <- convert_gene_name_in_uniprotid(ea_output, organism)

    if(!significant){
      output_uniprot_sub <- output_uniprot %>% dplyr::select(UNIPROT, gene_name, weightedNES, pvalues)
      unisign_uni <- convert_gene_name_in_uniprotid(dplyr::anti_join(output$all, output_uniprot, by = 'gene_name'),organism) %>%
        dplyr::rename('weightedNES' = 'NES')
      output_uniprot <- bind_rows(output_uniprot_sub, unisign_uni)
    }
  }else{
